...
class PostgreSQLMCPServer:
    """PostgreSQL MCP Server with HTTP/SSE transport."""
...
async def root():
    ...
    return {"status": "ok", "service": "postgresql-mcp-server"}


@app.get("/health")
async def health():
    """Health check with database status."""
    db_status = mcp_server.db.test_connection() if mcp_server.db else {"status": "not initialized"}
    return {
        "status": "ok",
        "database": db_status
    }
...
@app.get("/tools")
async def list_tools(authenticated: bool = Depends(auth_manager.verify_token)):
    """List available MCP tools."""
...
async def call_tool(
    tool_name: str,
    arguments: dict,
    authenticated: bool = Depends(auth_manager.verify_token)
):
    """Execute a tool."""
